...
static void ButtonTimerEventHandler(EventLoopTimer *timer)
{
    ...
    if (newButtonState != buttonState) {
        if (newButtonState == GPIO_Value_Low) {
            ...
            state = !state;
            int result = GPIO_SetValue(relayFd, state);
            if (result != 0) {
                Log_Debug("ERROR: Could not set Relay output value: %s (%d).\n", strerror(errno), errno);
                exitCode = ExitCode_LedTimer_SetLedState;
                return;
            }
        }
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    relayFd = GPIO_OpenAsOutput(SAMPLE_DEVICE_STATUS_LED, GPIO_OutputMode_PushPull, state);
    if (relayFd == -1) {
