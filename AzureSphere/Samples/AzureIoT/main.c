...
typedef enum {
    ...
    ExitCode_AccelTimerHandler_ReadStatus = 17,
    ExitCode_Init_AccelTimer = 18,
    ExitCode_Init_InitConfig = 19,
    ExitCode_Init_OpenSpiMaster = 20,
    ExitCode_Init_SetBusSpeed = 21,
    ExitCode_Init_SetMode = 22,
} ExitCode;
...
void updateRelayState(bool newState){
    state = newState;
    GPIO_SetValue(relayGpioFd, state);
    GPIO_SetValue(deviceTwinStatusLedGpioFd, !state);
    GPIO_SetValue(greenLedGpioFd, state);

    // Report current status LED state
    if (state) {
        TwinReportState("{\"locked\":true}");
    } else {
        TwinReportState("{\"locked\":false}");
    }

    uint8_t statusRegIdReadCmd = state ? 0x20 : 0x30;
    uint8_t status;
...
static void BlinkingLedTimerEventHandler(EventLoopTimer *timer)
{
    ...
    uint8_t statusRegIdReadCmd = 0x10;
    uint8_t status;
    ssize_t transferredBytes = SPIMaster_WriteThenRead(
        spiFd, &statusRegIdReadCmd, sizeof(statusRegIdReadCmd), &status, sizeof(status));
    if (!CheckTransferSize("SPIMaster_WriteThenRead (STATUS_REG)",
    ...
    if (status == 0x01) {
        newState = true;
        Log_Debug("Locked: true");
    }
    else if (status == 0x00) {
        newState = false;
        Log_Debug("Locked: false");
    }
    else {
        Log_Debug("Unknown response: %x", status);
    }
...
static void ButtonPollTimerEventHandler(EventLoopTimer *timer)
{
    ...
    if (IsButtonPressed(sendMessageButtonGpioFd, &sendMessageButtonState)) {
        ...
        GPIO_GetValue(relayGpioFd, &state);
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    deviceTwinStatusLedGpioFd =
        GPIO_OpenAsOutput(SAMPLE_LED, GPIO_OutputMode_PushPull, !state);
    if (deviceTwinStatusLedGpioFd == -1) {
        Log_Debug("ERROR: Could not open SAMPLE_LED: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_TwinStatusLed;
    }

    // SAMPLE_RGBLED_GREEN is used to show Device Twin settings state
    Log_Debug("Opening SAMPLE_RGBLED_GREEN as output.\n");
    greenLedGpioFd =
        GPIO_OpenAsOutput(SAMPLE_RGBLED_GREEN, GPIO_OutputMode_PushPull, state);
    if (greenLedGpioFd == -1) {
        Log_Debug("ERROR: Could not open SAMPLE_RGBLED_GREEN: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_TwinStatusLed;
    }

    // Open SAMPLE_DEVICE_STATUS_LED GPIO
    Log_Debug("Opening SAMPLE_DEVICE_STATUS_LED as output.\n");
    relayGpioFd = GPIO_OpenAsOutput(SAMPLE_DEVICE_STATUS_LED, GPIO_OutputMode_PushPull, state);
    if (relayGpioFd == -1) {
        Log_Debug("ERROR: Could not open SAMPLE_DEVICE_STATUS_LED GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_TwinStatusLed;
    }

    static const struct timespec timerCheckPeriod = {.tv_sec = 0, .tv_nsec = 500 * 1000 * 1000};
...
static void ClosePeripheralsAndHandlers(void)
{
    ...
    CloseFdAndPrintError(greenLedGpioFd, "GreenLed");
    CloseFdAndPrintError(relayGpioFd, "RelayLed");
                               ...
                               size_t payloadSize, void *userContextCallback)
{
    ...
    int statusLedValue = json_object_dotget_boolean(desiredProperties, "locked");
    if (statusLedValue != -1) {
        statusLedOn = statusLedValue == 1;
        updateRelayState(statusLedOn);
    }
